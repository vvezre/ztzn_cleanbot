...
@Slf4j
@Component
public class D12RailcarStrategy implements DeviceControlStrategy {
    ...
    public String encode(DeviceConfigRequest request) {
        try {
            ...
            appendIdentity(hex, request, "-D12");
            ...
            // H23-H34: D12 参数区
            hex.append(toWord(request.getControlMode(), 4));
            hex.append(toWord(request.getEnableMode(), 1));
            hex.append(toWord(request.getRobotInPositionTime(), 100));
            hex.append(toWord(request.getLimitPositionCheckTime(), 200));
            hex.append(toWord(request.getWalkPositionCheckTime(), 500));
            hex.append(toWord(request.getWalkFastSpeed(), 800));
            hex.append(toWord(request.getWalkSlowSpeed(), 400));
            hex.append(toWord(request.getMaxRowCount(), 10));
            ...
            log.error("D12 编码失败 - deviceId={}", request != null ? request.getDeviceId() : null, e);
            return null;
        }
    }
    ...
    private String finalizeHex(StringBuilder hex, DeviceConfigRequest request) {
        ...
        if (encoded.length() != FRAME_HEX_LENGTH) {
            log.warn("D12 编码长度异常 - deviceId={}, length={}, hex={}",
    ...
    private String resolveInteractionPayload(String payloadHex) {
        if (payloadHex != null) {
            ...
            log.warn("D12 交互区无效，使用全 0 兜底 - payloadHex={}", payloadHex);
        }
